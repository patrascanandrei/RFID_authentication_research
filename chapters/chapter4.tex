...
\section{Resetable tag privacy attack}

    In the case of resetable tags the adversary can revert them to their default values, meaning predictable behaviour of sent messages. These messages can be linked to particular
    ...
    \< \< \text{ \scriptsize E($N_t||S$, A, p)} \\[-3ex]
    % \\
    \< \sendmessageleft{top=\text{ \scriptsize 3.E($N_t||S$, A, p)}, bottom = \text{\scriptsize \colorbox{softred}{Intercepted by $A$}}}\\[-2ex]
    % \\
    \text{ \scriptsize .\ .\ .\ \ \ \ \ \ \ \ \ \ \ \ \ \ } \\[-1ex]
    % \\
    ...
    For resetable tags, the adversary eavesdrops on the output generated by the tag and links it to it. The Figure 3.4 presents how the adversary
